...
3. **校舎の詳細住所**
   現在は「阪急神戸線『塚口』駅より徒歩圏内（詳細住所は決定次第公開）」としています。
   決定後、お問い合わせ欄・アクセス欄・フッター・JSON-LDの `address` に反映してください。
...
5. **お問い合わせフォームの送信先**（下記参照）

## お問い合わせフォームの設定

初期状態では、送信ボタンを押すと**入力内容を差し込んだメール作成画面が開く**フォールバック動作になります。
`assets/js/main.js` 冒頭の定数を変更してください。

```js
const FALLBACK_MAIL_TO = 'info@example.com';   // ← 受信用メールアドレスに変更
```
